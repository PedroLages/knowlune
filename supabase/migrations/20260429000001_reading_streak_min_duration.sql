-- feat/bridge-books: Add 5-minute minimum duration to reading streak RPC.
--
-- Sessions with duration_seconds < 300 (5 minutes) are persisted and synced
-- but excluded from streak computation. This prevents micro-sessions (quick
-- opens, accidental taps) from inflating streak counts.
--
-- Uses CREATE OR REPLACE FUNCTION for idempotency.
--
-- R9: 5-minute minimum duration threshold for streak eligibility.
...
BEGIN
  ...
  WITH per_day AS (
    ...
    WHERE s.user_id = p_user_id
      AND s.duration_seconds >= 300  -- R9: 5-minute minimum for streak eligibility
    GROUP BY 1
  ),
  met_days AS (
    SELECT day
...
COMMENT ON FUNCTION public.compute_reading_streak(UUID, TEXT, TEXT, INT) IS
  'E95-S04 / R9: Aggregates public.study_sessions into (current_streak, longest_streak, last_met_date). 5-min minimum duration filter. Minutes goal only; pages deferred (OQ1). SECURITY INVOKER preserves RLS.';
