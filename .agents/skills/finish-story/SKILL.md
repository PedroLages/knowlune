...
14b. **Pattern extraction** (after lessons captured — skip if Step 14 was skipped):
   ...
   **Evaluate each substantive lesson** (skip trivial items like "used library X"):
   ...
   | **Rule** (`.Codex/rules/{area}.md`) | Process enforcement, quality constraint, or workflow rule that should be checked automatically | Append to existing rule file or create new file |
   ...
   **If candidates found:** Present via AskUserQuestion:
   ...
   2. [rule] "Rule Title" → .Codex/rules/{area}/{file}.md
   ...
   **Apply approved suggestions:**
   - **Patterns**: Append to end of `docs/engineering-patterns.md` using the existing format:
     ...
     **Case study**: E##-S## — [brief description of how this was discovered]
     ```
   - **Rules**: Append to the most relevant existing rule file in `.Codex/rules/`, or create a new file if no existing file fits
   - **Memory**: Write memory entry using the auto-memory system (frontmatter format to `~/.Codex/projects/.../memory/`)

   **Commit** (if any suggestions were applied):
   ```bash
   git add docs/engineering-patterns.md .Codex/rules/
