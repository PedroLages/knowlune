...
See: `.Codex/skills/review-story/config/gates.json`
...
MANIFEST=$(python3 scripts/workflow/prepare-dispatch.py \
  ...
  --resuming=$RESUMING \
  --gates-config=.Codex/skills/review-story/config/gates.json)
...
echo "$AGENT_RETURN" | jq '.' > "${BASE_PATH}/.Codex/state/review-story/agent-results/{gate}-${STORY_ID}.json"
```
