...
- **Validate localStorage on read, not just write**: The code review caught that `parseFloat` on localStorage can produce any float, making `indexOf` return -1. Adding a nearest-speed fallback guard and validating against the allowed array on init prevents corrupted state from propagating.
- **Global DOM queries are fragile**: `document.querySelector('[contenteditable="true"]')` grabs the first match anywhere — scoping to `.ProseMirror` within the notes container is more resilient to future UI additions.
- **Overlay separator semantics matter**: Showing `< + >` implies a chord (simultaneous press); using `/` as separator communicates independent keys. Small UX detail caught by both design and code review.
