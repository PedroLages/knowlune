...
- [ ] Task 1: Add `calculateItemDifficulty` to `src/lib/analytics.ts` (AC: 1, 2, 3, 4)
  - [ ] 1.1 Define `ItemDifficulty` type: `{ questionId, questionText, pValue, difficulty, topic, order }`
  - [ ] 1.2 Implement aggregation loop across all attempts and answers
  - [ ] 1.3 Apply difficulty categorization (P >= 0.8 = Easy, 0.5 <= P < 0.8 = Medium, P < 0.5 = Difficult)
  - [ ] 1.4 Exclude questions with zero attempts (filter nulls)
  - [ ] 1.5 Sort results easiest-first (descending P-value)

- [ ] Task 2: Write unit tests for `calculateItemDifficulty` (AC: 1, 2, 3, 4)
  - [ ] 2.1 P-value calculation accuracy (3/4 = 0.75)
  - [ ] 2.2 Difficulty categorization boundaries (P=0.8 is Easy, P=0.79 is Medium, P=0.5 is Medium, P=0.49 is Difficult)
  - [ ] 2.3 Questions with zero attempts excluded
  - [ ] 2.4 Sort order: easiest first (highest P-value first)
  - [ ] 2.5 Empty attempts array returns empty result
  - [ ] 2.6 Single attempt with all correct returns all Easy
  - [ ] 2.7 Aggregation works across multiple attempts for same question

- [ ] Task 3: Create `ItemDifficultyAnalysis.tsx` component (AC: 1, 3, 4, 5)
  - [ ] 3.1 Accept `quiz: Quiz` and `attempts: QuizAttempt[]` props
  - [ ] 3.2 Empty state: "Not enough data to analyze difficulty." when no questions qualify
  - [ ] 3.3 Render question list with difficulty badge and P-value
  - [ ] 3.4 Color-code badges: Easy=success, Medium=warning, Difficult=destructive
  - [ ] 3.5 Suggestion text for difficult questions (P < 0.5): show topic and percentage
  - [ ] 3.6 Truncate long question text with `truncate`

- [ ] Task 4: Write component tests for `ItemDifficultyAnalysis` (AC: 1, 3, 4, 5)
  - [ ] 4.1 Renders empty state when no attempts
  - [ ] 4.2 Renders question list sorted easiest-first
  - [ ] 4.3 Shows correct difficulty badge for each category
  - [ ] 4.4 Suggestion text appears only for Difficult questions

- [ ] Task 5: Integrate into `QuizResults.tsx` (AC: 1)
  - [ ] 5.1 Import `ItemDifficultyAnalysis` component
  - [ ] 5.2 Render after `ScoreTrajectoryChart` (only when 2+ attempts exist for meaningful analysis)
  - [ ] 5.3 Pass `currentQuiz` and `attempts` as props

- [ ] Task 6: Write E2E test (AC: 1, 3)
  - [ ] 6.1 Seed quiz and 3 attempts with varied per-question performance
  - [ ] 6.2 Navigate to `/courses/:courseId/lessons/:lessonId/quiz/results`
  - [ ] 6.3 Assert "Question Difficulty Analysis" section visible
  - [ ] 6.4 Assert difficulty labels (Easy/Medium/Difficult) visible
