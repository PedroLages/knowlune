...
- **addInitScript re-runs on navigation**: Playwright's `page.addInitScript()` re-executes on every navigation, which can overwrite localStorage values a test just set. Assert persistence state before navigating, not after, to avoid the script clobbering the value under test.
- **Grid layout should adapt to visible children**: When feature toggles hide grid items, static `grid-cols-N` leaves empty cells. Derive the column count from visible item count (e.g., conditional `lg:grid-cols-[3fr_2fr]`) to prevent layout gaps.
