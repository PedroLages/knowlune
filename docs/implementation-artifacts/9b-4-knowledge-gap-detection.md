...
- **Severity color scope**: Design review caught that severity color classes (`text-destructive`) bled into child elements including the video title heading, reducing contrast to ~4.3:1. Fix: scope color to badge only, keep card text in `text-foreground`. Lesson: always test colored card variants with real content, not just the badge.
- **Recurring `cn()` pattern**: String interpolation for className was flagged again (H1 in code review, recurring since E01-S03). The `cn()` utility from class-variance-authority merges conflicting Tailwind classes correctly — template literals don't. This should be second nature by now.
- **Bidirectional writes need transactions**: Two `db.notes.put()` calls in `Promise.all` can leave broken one-way links if the second fails. Wrapping in `db.transaction('rw', db.notes, ...)` ensures atomicity. Apply this pattern to any multi-record Dexie write.
