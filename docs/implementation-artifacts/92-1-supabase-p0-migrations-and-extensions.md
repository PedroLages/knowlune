...
**Cross-model severity disagreement is a signal, not noise.** The duplicate `_status_rank` definition was rated NIT by Claude, HIGH by GLM, and BLOCKER by the OpenAI fallback. The correct severity was somewhere in the middle — latent footgun, not a runtime bug. When three reviewers disagree widely on a single finding, that spread itself is a flag: the code is ambiguous enough that a future maintainer will also be confused. Fixed in R3. For future schema stories: when cross-model consensus flags the same issue at varying severities, fix it regardless of the lowest rating.
