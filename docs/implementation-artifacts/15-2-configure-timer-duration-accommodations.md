...
- **Fix-introduces-anti-pattern cycle**: The v1 code review caught an empty catch block in Quiz.tsx, which was fixed with `console.warn`. But the fix for a different finding (extracting shared logic) introduced the same empty-catch anti-pattern in QuizResults.tsx. Lesson: when fixing a pattern issue, grep for the same anti-pattern in all files touched by the fix, not just the originally flagged file.
- **Extract on second use, not after review**: The shared `loadSavedAccommodation` utility should have been extracted when QuizResults.tsx first duplicated the logic from Quiz.tsx — not after the code review flagged it. "Don't repeat yourself" at implementation time saves a review round.
