...
- **React automatic batching catches synchronous multi-fire**: The blocker (all 3 thresholds firing in one tick for short quizzes) exposed how React's automatic batching (introduced in React 18, present in our React 19) silently drops intermediate state updates. Fix: fire toasts imperatively in the callback rather than routing through useState. Lesson: when multiple state transitions can occur in a single synchronous block, use refs or imperative calls instead of state.
- **Persistent toasts need explicit cleanup**: Sonner toasts with `duration: Infinity` survive route changes because `<Toaster />` lives in the root layout. Must store the toast ID and call `toast.dismiss(id)` on quiz end. Lesson: any non-auto-dismissing UI element needs an explicit teardown path.
- **Duplicate ARIA live regions compound cognitive load**: Both `QuizTimer` (pre-existing) and `TimerWarnings` (new) announced at the 25% threshold, doubling screen reader output at a stressful moment. Consolidating threshold announcements into a single component prevents this. Lesson: audit existing ARIA regions before adding new ones to the same page.
