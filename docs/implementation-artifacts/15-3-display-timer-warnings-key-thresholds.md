...
- **React 18 batching catches synchronous multi-fire**: The blocker (all 3 thresholds firing in one tick for short quizzes) exposed how React 18's automatic batching silently drops intermediate state updates. Fix: fire toasts imperatively in the callback rather than routing through useState. Lesson: when multiple state transitions can occur in a single synchronous block, use refs or imperative calls instead of state.
