...
- **Fake timers + async persistence**: `persistWithRetry` + `vi.useFakeTimers` causes unhandled rejections. Mock the persistence layer at module level with `vi.mock` instead.
- **E2E IDB cleanup**: IndexedDB `deleteDatabase` uses callbacks, not promises. Wrap in a Promise and `await` in `afterEach` to prevent flaky inter-test pollution.
- **Optimistic rollback**: Snapshot the full state array before mutation. Reconstructing state after failure (e.g., appending a deleted item) loses ordering.
- **Three review rounds**: First review caught error swallowing and timezone bugs. Second caught test gaps. Third was clean — iterative review cycles compound quality.
- **Type guard edge cases**: When form state allows empty `type`, validation messages referencing `UNIT_LABELS[type]` produce "undefined" strings. Always guard with a type check before lookup.
- **useEffect cleanup flags**: Ignore flags in effects without async operations are misleading dead code. Remove them to avoid false confidence in race condition handling.
- **Design review catches a11y regressions**: Stale `aria-invalid` attributes persist when validation errors aren't cleared on field change — caught by design review, not unit tests.
- **Overview page redesign ripple**: Changing the Overview page layout broke smoke test selectors across multiple specs. Run full smoke suite early when touching shared pages.
