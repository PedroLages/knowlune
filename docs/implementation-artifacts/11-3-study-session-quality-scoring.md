...
|-----------|------|-------|
| QualityScoreDialog | *(removed)* | Was `src/app/components/session/QualityScoreDialog.tsx` |
| QualityScoreRing | *(removed)* | Was `src/app/components/session/QualityScoreRing.tsx` |
| FactorBreakdown | *(removed)* | Was `src/app/components/session/FactorBreakdown.tsx` |
| TrendIndicator | `src/app/components/session/TrendIndicator.tsx` | Session history trend |
| SessionHistory | `src/app/pages/SessionHistory.tsx` | Score column + trend |
| Quality score types | `src/data/types.ts` | Quality fields on `StudySession` |
| Score calculation | `src/lib/qualityScore.ts` | Pure scoring logic |
| Session store | `src/stores/useSessionStore.ts` | Computes score on `endSession`; no popup `CustomEvent` |
...
- **Session History as primary UI**: Quality scores render in `SessionHistory` (badge + trend). The post-session **QualityScoreDialog** and **`session-quality-calculated` → Layout** wiring were removed; scoring still runs for eligible sessions so history stays populated.
- **Responsive dialog pattern**: *(Historical)* QualityScoreDialog used `useIsMobile()` — removed with dialog.
...
- **E2E tests** (`story-e11-s03.spec.ts`): Covers ACs via **Session History** — seeded `studySessions` rows with `qualityScore` / `qualityFactors`, asserts badges and trend indicator (`tests/e2e/regression/story-e11-s03.spec.ts`).
...
- **CustomEvent / dialog testing** — *(Historical)* Playwright dispatched `session-quality-calculated` to exercise the post-session dialog without the full lesson player. Current E2E targets **Session History** rows instead after dialog removal.
- **Mobile-first dialog rendering** — *(Historical)* QualityScoreDialog used `useIsMobile()` for Sheet vs Dialog; removed with the popup flow.
