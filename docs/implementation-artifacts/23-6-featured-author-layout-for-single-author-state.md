...
- **Commit scope discipline:** Quiz radio-group fixes (WAI-ARIA selection-follows-focus) landed in E23-S06 fix commits — unrelated changes that complicate revert and bisect. Keep story commits scoped to the story's feature area only.
- **Consolidation requires full sweep:** Extracting `getInitials` to `@/lib/textUtils` missed `AuthorProfile.tsx` which kept its own inline version with different behavior (no null guard, no 2-initial cap). When consolidating utilities, grep all consumers and verify behavioral parity.
- **Re-export traps:** Leaving a re-export of `getInitials` in `avatarUpload.ts` after migration creates a ghost dependency — future consumers can import from the old path without error. Remove re-exports after migration to surface breakage at build time.
