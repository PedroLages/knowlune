...
| `ai_embeddings`          | Delete all embedding rows from `embeddings` table. Mark all `learnerModels` rows with `frozenReason: 'consent_withdrawn'` (TypeScript field added in E119-S08; Dexie schema v59). | Embeddings are NOT auto-regenerated. Learner model un-freezes on next content write. |
| `voice_transcription`    | **No-op (E119-S08):** No audio job queue table exists in the current implementation. Past transcripts are retained as part of the user's notes. This will be revisited when an audio processing queue is introduced. | Immediately available; new recordings are processed.            |
