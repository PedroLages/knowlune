...
Users creating and navigating learning tracks encounter several UX friction points: no visual position indicators when selecting courses during creation, no drag-and-drop for reordering, a sticky progress sidebar that scrolls with the page, the first syllabus module expanded by default, and a CTA button that navigates to the course overview instead of directly to the lesson.
...
- R6. The hero banner "Start Learning" / "Continue Learning" button navigates directly to the first incomplete video lesson within the target course. If no progress exists, it navigates to the first lesson of the first course.
