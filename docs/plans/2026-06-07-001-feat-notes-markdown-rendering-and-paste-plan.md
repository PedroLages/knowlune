...
- **Native `getMarkdown()` for single-active-note export/copy:** Could replace Turndown for the _live editor_ case (e.g. a "Copy as Markdown" button) in a future iteration. Bulk/at-rest export in `src/lib/noteExport.ts` would still need Turndown (or a headless `generateJSON` + `MarkdownManager` serializer), so this is out of scope here.
