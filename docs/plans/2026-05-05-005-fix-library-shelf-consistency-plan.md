...
- **Corner clipping fix**: Add `[transform:translateZ(0)]` or `isolate` to the cover container to force a new stacking context, ensuring the parent's `overflow-hidden rounded-2xl` clips the scaled child image correctly during GPU-composited transforms. This is a known browser compositing issue with nested transforms.
- **Icon overlay instead of text**: Replace the text `<span>` in the overlay with a single large action icon (`PlayCircle` for audiobooks — suggesting "tap to play", `BookOpen` for ebooks — suggesting "tap to read"). This is an action hint, not a format indicator (format is shown via the top-right Headphones/BookOpen badge on BookCard, and the overlay icon serves a different purpose). The icon acts as the click target hint.
...
- Fix corner clipping: add `isolate` to the cover container to force a stacking context that correctly clips composited child transforms to `rounded-2xl`.
- Remove the "Audio" badge (top-left) — it becomes redundant once the format icon is in the overlay (Unit 2) and the Browse cards get their own top-right icon (Unit 4).
...
- Edge case: hover scale does not produce visible sharp corners (verify `isolate` stacking context fix)
