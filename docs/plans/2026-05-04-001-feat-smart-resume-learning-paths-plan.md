...
- **Per-path hook (not cross-path):** The hook takes a single `pathId`. Cross-path ranking (R11-R12) is deferred; the Overview section iterates over available paths using this hook.
- **Navigation target for path card "Continue":** Navigates to `/courses/${courseId}` (course overview page) for MVP. This bypasses the path detail page (2 steps instead of 4) while avoiding the need for per-lesson progress lookup at the path card level. Direct lesson player navigation can be refined in a follow-up.
...
- Return shape: `{ entry: LearningPathEntry | null, course: ImportedCourse | null, action: 'resume' | 'start' | 'complete' | null }`
...
- Create a new component `ContinueLearningPathSection` that:
  - Uses `useLearningPathStore(s => s.paths)` to get all user paths
  - For each path, calls `useNextBestCourse(path.id)` to get the next course
  - Filters to paths that have a non-null result (ignores empty or complete paths with no action)
...
- "Continue" button navigates to `/courses/${courseId}` where `courseId` comes from the `useNextBestCourse` result
...
- Happy path: Clicking "Continue" navigates to `/courses/{courseId}`
...
- In `PathCard`, replace the current footer (`ArrowRight` / "Not Started" / "Review") with:
  - **In-progress (action='resume'):** "Continue 'Course Name'" Button — navigates to `/courses/${courseId}`
  - **Unstarted (action='start'):** "Start 'Course Name'" Button — navigates to `/courses/${courseId}`
...
- Modify: `src/app/hooks/useCompletionFlow.ts` (pass path context to the course suggestion trigger)
- Test: `src/app/pages/__tests__/UnifiedLessonPlayer.test.tsx` (extend existing)

**Approach:**
- Create `NextInPath` component:
  - Props: `{ pathName: string, courseName: string, isLastInPath: boolean, onNavigate: () => void, onDismiss: () => void }`
  - Shown after course-level celebration modal closes (same trigger point as existing `NextCourseSuggestion`)
  - For non-last course: shows "Next in path: [Course Name]" with a "Continue" button that navigates to the next course
  ...
  - Uses `useLearningPathStore` to find which path(s) contain the completed course
  - Uses `useNextBestCourse` to find the next course in that path
- In `UnifiedLessonPlayer.tsx`:
  - After the completion modal closes (`showCourseSuggestion` becomes true), check if the completed course belongs to a path
  - If yes, render `NextInPath` instead of `NextCourseSuggestion`
  - If no, render the existing `NextCourseSuggestion` (current behavior unchanged)
- In `useCompletionFlow.ts`:
  - Add a `pathContext` param to `CompletionFlowParams` (optional: `{ pathId: string, pathName: string }`)
  - Pass it through so the lesson player knows which path context to use
  - Wire `handleCelebrationOpenChange` to set a new `nextInPathOpen` state in addition to the existing `showCourseSuggestion`
...
| Path card "Continue" navigation to course overview (not lesson player) may not satisfy "1 click" requirement | Documented as MVP scope decision; direct lesson player navigation is a follow-up |
