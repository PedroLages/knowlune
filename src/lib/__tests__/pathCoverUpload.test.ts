...
const {
  ...
  const mockGetSession = vi.fn().mockResolvedValue({
    data: {
      session: { access_token: 'valid-token', expires_at: Math.floor(Date.now() / 1000) + 3600 },
    },
    error: null,
  })
  ...
  describe('uploadPathCover', () => {
    ...
    it('throws a user-friendly error when 409 retry remove fails', async () => {
      ...
      mockRemove.mockResolvedValueOnce({
        error: { message: 'Remove permission denied', statusCode: 403 },
      })
    ...
    it('rejects when user is not authenticated', async () => {
      ...
      const file = new File([new Uint8Array([1])], 'cover.png', { type: 'image/png' })
      await expect(uploadPathCover(file, PATH_ID)).rejects.toThrow(
        /Sign in required to upload covers/
      )
