...
describe('calculateNormalizedGain', () => {
  ...
  it('returns null for NaN inputs (guard against corrupted data)', () => {
    expect(calculateNormalizedGain(NaN, 80)).toBeNull()
    expect(calculateNormalizedGain(60, NaN)).toBeNull()
  })

  it('returns null for Infinity inputs (guard against corrupted data)', () => {
    expect(calculateNormalizedGain(Infinity, 80)).toBeNull()
  })
  ...
  it('returns 0 when scores are identical', () => {
    ...
    expect(calculateNormalizedGain(0, 0)).toBe(0)
  })
  ...
  it('returns 1 for improvement to 100 from 99 (boundary is exclusive)', () => {
    // (100 - 99) / (100 - 99) = 1 — confirms initialScore=99 is NOT blocked by guard
    expect(calculateNormalizedGain(99, 100)).toBeCloseTo(1.0)
  })
...
describe('interpretNormalizedGain', () => {
  ...
  it('returns exact message strings for all four levels', () => {
    expect(interpretNormalizedGain(-0.5).message).toBe(
      'Score decreased — review the material and try again!'
    )
    expect(interpretNormalizedGain(0.1).message).toBe("You're making progress. Keep practicing!")
    expect(interpretNormalizedGain(0.5).message).toBe('Good learning progress!')
    expect(interpretNormalizedGain(0.9).message).toBe('Excellent learning efficiency!')
  })
})
