...
export async function exportPkmBundle(
  ...
  const noteFiles = await exportNotesAsMarkdown((percent, phase) => {
  ...
  const flashcardFiles = await exportFlashcardsAsMarkdown((percent, phase) => {
  ...
  const bookmarkFiles = await exportBookmarksAsMarkdown((percent, phase) => {
