...
function handleMetric(metric: Metric) {
  ...
  if (isDev) {
    ...
    console.log(
      `[Knowlune:Perf] ${metric.name}: ${metric.value.toFixed(2)}${unit} (rating: ${metric.rating})`
    )
  }
}
...
export function markRouteStart(routeName: string) {
  ...
  if (isDev) {
    console.log(`[Knowlune:Perf] Route transition started: ${routeName}`)
  }
}
...
export function markRouteEnd(routeName: string) {
  ...
  try {
    ...
    if (isDev) {
      console.log(`[Knowlune:Perf] Route transition ${routeName}: ${measure.duration.toFixed(2)}ms`)
    }
    ...
    if (isDev) {
      console.warn(`[Knowlune:Perf] Missing start mark for route: ${routeName}`)
    }
  } finally {
    // Clean up marks
...
export function initPerformanceMonitoring() {
  ...
  if (isDev) {
    console.log('[Knowlune:Perf] Performance monitoring initialized')
  }
}
