 ...
 *
 * Sequence:
 *   1. `syncCoordinator.stop()`           — halt queued work + pending debounce.
 ...
 *      `syncMetadata` cursors so the next coordinated run re-downloads from t=0.
 *   4. `syncCoordinator.start(userId)`    — triggers the initial full sync that
...
export async function resetLocalData(userId: string | null): Promise<void> {
  ...
  try {
    ...
    console.warn('[resetLocalData] syncCoordinator.stop() threw:', err)
  }
  ...
  //    coordinated sync re-downloads from the beginning instead of from a stale
  ...
  if (userId) {
    try {
      await syncCoordinator.start(userId)
    } catch (err) {
      // Surfaced to caller via rejection — the UI layer shows a toast.
      console.error('[resetLocalData] syncCoordinator.start failed after reset:', err)
      throw err
    }
  }
}
