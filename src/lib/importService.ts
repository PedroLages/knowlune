/**
 * Data import service for EduVi learning platform.
 *
 * Re-imports a previously exported EduVi JSON file into IndexedDB + localStorage.
...
import { CURRENT_SCHEMA_VERSION, type EduViExport } from './exportService'
...
function isValidExport(data: unknown): data is EduViExport {
...
type MigrationFn = (data: EduViExport) => EduViExport
...
function applyMigrations(data: EduViExport): EduViExport {
  let current = data
...
export async function importFullData(json: string): Promise<ImportResult> {
  ...
  if (!isValidExport(parsed)) {
    return {
      success: false,
      recordCount: 0,
      error: 'Not a valid EduVi export file (missing schemaVersion or data)',
    }
  }

  let exportData = parsed as EduViExport
