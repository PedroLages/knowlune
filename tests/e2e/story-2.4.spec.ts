...
const COURSE_ID = 'op6-course'
const LESSON_WITH_PDF = 'op6-introduction' // has both video + PDF
...
test.describe('AC1: PDF Rendering and Page Navigation', () => {
  ...
  test('should display page navigation showing current page and total pages', async ({
    ...
    // GIVEN: PDF viewer is rendered
    await navigateAndWait(page, LESSON_PLAYER_URL)
    await page.getByRole('tab', { name: /materials/i }).click()

    // THEN: Page indicator shows "Page X of Y" format
    const pageIndicator = page.getByTestId('pdf-page-indicator')
    await expect(pageIndicator).toBeVisible()
    await expect(pageIndicator).toContainText(/\d+\s*(\/|of)\s*\d+/)
  })

  test('should navigate to next page when clicking next button', async ({ page }) => {
    // GIVEN: PDF viewer on page 1
    await navigateAndWait(page, LESSON_PLAYER_URL)
    await page.getByRole('tab', { name: /materials/i }).click()
  ...
  test('should navigate to previous page when clicking prev button', async ({ page }) => {
    ...
    await page.getByTestId('pdf-prev-page').click()
  ...
  test('should support keyboard navigation with PageUp', async ({ page }) => {
    // GIVEN: PDF viewer focused on page 2
    await navigateAndWait(page, LESSON_PLAYER_URL)
    await page.getByRole('tab', { name: /materials/i }).click()
  ...
  test('should navigate to first page with Home key', async ({ page }) => {
    // GIVEN: PDF viewer on a later page
    await navigateAndWait(page, LESSON_PLAYER_URL)
    await page.getByRole('tab', { name: /materials/i }).click()
  ...
  test('should navigate to last page with End key', async ({ page }) => {
    ...
    const pageInput = page.getByTestId('pdf-page-input')
    const totalPages = page.getByTestId('pdf-total-pages')
...
test.describe('AC2: Zoom Controls and Text Selection', () => {
  ...
  test('should have fit-width option in zoom controls', async ({ page }) => {
    // GIVEN: PDF viewer rendered
    await navigateAndWait(page, LESSON_PLAYER_URL)
    await page.getByRole('tab', { name: /materials/i }).click()
  ...
  test('should have fit-page option in zoom controls', async ({ page }) => {
    // GIVEN: PDF viewer rendered
    await navigateAndWait(page, LESSON_PLAYER_URL)
    await page.getByRole('tab', { name: /materials/i }).click()
