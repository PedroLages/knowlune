 ...
 *
 * Tests:
 * 1. DB schema v9 with embeddings table (AC1)
 * 2. DB internal version is 90 (Dexie v9 × 10) (AC1)
...
test.describe('E09-S03: Embedding Pipeline & Vector Store', () => {
  ...
  test('schema v9: embeddings table exists in IndexedDB', async ({ page }) => {
  ...
  test('schema v9: database internal version is 90 (Dexie v9 × 10)', async ({ page }) => {
    await page.goto('/notes')
    await page.waitForLoadState('networkidle')
    ...
    // Dexie multiplies schema version by 10 internally — Dexie v9 = IDB v90
  ...
  test('semantic search displays similarity badges when active', async ({ page }) => {
    // Navigate first — IndexedDB is only accessible after navigating to the app origin
    await page.goto('/notes')
    await page.waitForLoadState('networkidle')
