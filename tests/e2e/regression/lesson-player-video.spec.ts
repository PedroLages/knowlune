...
test.describe('AC-1: Video Playback from Imported Course', () => {
  ...
  test('should display video title in header', async ({ page, indexedDB }) => {
    ...
    // THEN: Video title is displayed below the video
    await expect(page.getByTestId('lesson-title')).toContainText('01-Introduction.mp4')
  })

  test('should display course name in back-link toolbar', async ({ page, indexedDB }) => {
    // GIVEN: Imported course with videos seeded
    await seedCourseAndReload(page, indexedDB)
    ...
    // THEN: Course name is displayed in the back-link
    await expect(page.getByLabel('Back to course')).toContainText('React Fundamentals')
  })
